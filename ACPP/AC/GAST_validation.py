...
def start_validation(sol, instances,
    ...
    while True:
        ...
        cmd = 'python '+os.path.abspath(os.path.realpath(__file__)+"/../../../")+('/ACPP/src/util/validation_wrapper.py %s %s %d %d %d %s') % (instance, output_file, cutoffTime, cutoff_length, seed, sol)
...
if __name__ == "__main__":
    ...
    seed_index_file = (file_path+"/ACPP/validation_output/GLOBAL/seed_index.txt")
    ...
    outputdir = glob.glob((file_path+
                           "/ACPP/AC_output/GAST/run%d/output/run%d/"
                           "log-run*.txt") %
    ...
    outputdir = (file_path+
                 "/ACPP/validation_output/GLOBAL/run%d") % run_number
