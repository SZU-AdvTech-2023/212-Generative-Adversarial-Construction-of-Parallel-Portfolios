...
Invoke like:
python testing_wrapper.py <testing_instance_file> <output file>
...
sys.path.append('/home/liuwei/GAST/ACPP/src/util')
...
class testing(object):
    def __init__(self):
        ...
        self._log_path = sys.path[0] + "/testlogs/"
    ...
    def __construct_cmd(self, solver_path, parameter_list):
        # for plingeling no seed allowed
        if 'plingeling' in solver_path or 'pfolioUZK-master/pfolioUZK'\
                                          in solver_path:
            cmd = solver_path + " "
        ...
        if 'jikken' in solver_path:
            optimumFile = ('/home/liusc/projects/ACPP/instance_set/TSP/indices/'
                           'TSP_optimum.json')
            if 'AC_output/GAST' in self._instance:
                values = self._instance[self._instance.find(
                    'it')+2:].strip().split('/')
                if 'n' not in values[1]:
                    optimumFile = ('/home/liusc/projects/ACPP/AC_output/GAST/'
    ...
    def __read_output(self, runtime):
        ...
        for thread_id, log in self._solver_log.items():
            detailed_results[thread_id] = 'TIMEOUT'
                    ...
                    for line in log.readlines():
                        if line.startswith("s "):
                            if "UNSATISFIABLE" in line:
                                ...
                                result = "UNSAT"
                                detailed_results[thread_id] = 'UNSAT'
                            elif "SATISFIABLE" in line:
                                self._watcher_log.write(
                                    str(thread_id) + ": solved\n")
                                result = "SAT"
                                detailed_results[thread_id] = 'SAT'
        ...
        return output_line, detailed_results
    ...
    def start(self):
        ...
        runtime = exe.run()
        output_line, detailed_results = self.__read_output(runtime)
        with open(self._output_file, "w+") as f:
            f.write(output_line + '\n')
            for thread_id, r in detailed_results.items():
                f.write('%s:%s ' % (str(thread_id), r))
