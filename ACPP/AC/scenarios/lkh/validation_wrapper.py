...
#sys.path.append('/home/liusc/projects/ACPP')
...
class testing(object):
    ...
    def parse_args(self, argv):
        ...
        [self._thread_to_solver, self._thread_to_params, _] = parser.parse(
            self._params)
        print(self._thread_to_solver)
        print(self._thread_to_params)
        exit()
    def __construct_cmd(self, solver_path, parameter_list):
        ...
        if 'LKH' in solver_path:
            optimumFile = ('/home/tsinny/shared/TSP_features/ACPP/AC/scenarios/lkh/'
                           'TSP_optimum.json')
            if 'AC_output/GAST' in self._instance:
                values = self._instance[self._instance.find(
                    'it')+2:].strip().split('/')
    ...
    def start(self):
        ...
        self._watcher_log = NamedTemporaryFile(mode="w+b", prefix="watcher", dir=self._watcher_path)
