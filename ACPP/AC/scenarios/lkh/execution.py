...
class execution(object):
    ...
    def run(self):
        ...
        #exit()
        self._watcher_log.write("Parent process PID is " + str(self._sub_process[1].ppid()) + "\n")
                        ...
                        if content.find('Successes/Runs'):
                            ...
                            if rrr:
                                if int(rrr.group()) > 0:
                                    ...
                                    self._watcher_log.write('Reason: success')
                                    self._watcher_log.flush()
                                    line=content[content.find('Time.avg'):]
                                    line=re.sub(r'Time.avg = ',"",line)
                                    actual_time=float(re.sub(r' sec\.,.*$',"",line))
                                    #exit()
                                    print("found")
                                    #exit()
                                    break
        ...
        self.__terminate()
        #print(min(self._cutoff_time, elapsedTime,actual_time))
        #exit()
        return min(self._cutoff_time, elapsedTime,actual_time)
