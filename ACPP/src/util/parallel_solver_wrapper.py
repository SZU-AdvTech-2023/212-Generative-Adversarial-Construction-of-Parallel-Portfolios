...
Smac/ParamilS.
Command Line:
python parallel_solver_wrapper.py <instance name>
...
class parallel_solver_wrapper(object):
    ...
    def parse_args(self, argv):
        ...
        self._instance_specific = argv[2]
    ...
    def __construct_cmd(self, solver_path, parameter_list):
        ...
        if 'LKH' in solver_path:
            ...
            #tmpfile = os.path.dirname(os.path.realpath(__file__))+'/zql'+str(len(self.paramFile))
            #tmp=open(tmpfile,'wb')
            self.paramFile[len(self.paramFile)] = tmp#tmpfile
            tmp.write('PROBLEM_FILE=%s\n' % self._instance)
            tmp.write('OPTIMUM=%s\n' % optimum[self._instance])
            #tmp.write('TIME_LIMIT=%s\n' % str(10*self._cutoff_time))
            for parameter in parameter_list:
                tmp.write(parameter[1:] + '\n')
            tmp.write('SEED=%s\n' % self._random_seed)
            tmp.flush()
            #tmp.close()
            cmd = solver_path + ' ' + tmp.name#tmpfile#tmp.name
            #print(cmd)
            return cmd
	...
	#exit()
        ...
        runtime = exe.run()
        print self.__read_output(runtime)
